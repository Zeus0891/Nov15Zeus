 ...
 *
 * SOURCE OF TRUTH: RBAC_Matrix.md
...
function validateSchema(schema: RBACSchema): boolean {
  ...
  if (schema.phase !== "PHASE_1_INTERNAL_MEMBERS") {
    log.error(
      `Invalid phase: ${schema.phase}. Expected: PHASE_1_INTERNAL_MEMBERS`
    );
    isValid = false;
  } else {
  ...
  if (schema.source_document !== "RBAC_Matrix.md") {
  ...
  if (JSON.stringify(roleCodes) !== JSON.stringify(expectedRoles)) {
    log.error(
      `Invalid role codes. Expected: ${expectedRoles}, Found: ${roleCodes}`
    );
    isValid = false;
  } else {
  ...
  if (externalRoles.length > 0) {
    log.error(
      `Phase 1 should only have INTERNAL roles. Found EXTERNAL: ${externalRoles
        .map((r) => r.code)
        .join(", ")}`
    );
    isValid = false;
  ...
  if (invalidPermissions > 0) {
    log.warn(
      `Found ${invalidPermissions} permissions with non-standard format`
    );
  } else {
  ...
  if (wildcards.length > 0) {
    log.error(
      `Found wildcards (not allowed in Phase 1): ${wildcards.join(", ")}`
    );
    isValid = false;
  } else {
  ...
  if (!schema.permission_catalog) {
    ...
    const catalogSize = Object.values(schema.permission_catalog).flat().length;
  ...
  if (schema.critical_permissions) {
    const criticalPerms = [
      ...schema.critical_permissions.PROJECT_MANAGER_ELEVATED.approvals.map(
        (p) => p.permission
      ),
      ...schema.critical_permissions.PROJECT_MANAGER_ELEVATED.deletes.map(
        (p) => p.permission
      ),
      ...schema.critical_permissions.PROJECT_MANAGER_ELEVATED.financial_visibility.map(
        (p) => p.permission
      ),
    ];
    ...
    if (criticalInGrants.length > 0) {
      log.error(
        `Critical permissions should NOT be granted to PM by default: ${criticalInGrants.join(
          ", "
        )}`
      );
      isValid = false;
    ...
    const missingInCatalog = criticalPerms.filter(
      (p) => !catalogPerms.includes(p)
    );

    if (missingInCatalog.length > 0) {
      log.error(
        `Critical permissions missing from catalog: ${missingInCatalog.join(
          ", "
        )}`
      );
      isValid = false;
...
${result.criticalPermissions
  .filter(
    (p) =>
      p.includes(":read:profit") ||
      p.includes(":read:financial") ||
...
function generatePermissionConstants(
  permissions: PermissionMetadata[]
): string {
...
async function main() {
  ...
  log.info(`Source: RBAC_Matrix.md`);
  ...
  try {
    ...
    log.success(
      `Critical Permissions (PM): ${result.validation.critical_permissions_count}`
    );
    ...
    log.info(
      "  4. Implement TenantSettings checks for PM critical permissions"
    );
  } catch (error) {
