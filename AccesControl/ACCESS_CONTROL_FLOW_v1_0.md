...
export async function checkPermission(
  ...
  try {
    ...
    for (const memberRole of memberRoles) {
      ...
      rolePermissions.forEach(p => allPermissions.add(p.permission.permissionCode));
    }
...
//   resource: { totalAmount: 35000, status: 'PENDING_APPROVAL' }
//
// Result:
...
async function authenticateApiKey(
  ...
  for (const keyRecord of activeKeys) {
    ...
    if (isMatch) {
      ...
      if (keyRecord.allowedIpAddresses && keyRecord.allowedIpAddresses.length > 0) {
...
async function generateSOXComplianceReport(
  ...
  const userAccessChanges = await prisma.accessAuditEvent.findMany({
    where: {
      ...
      actionType: {
        in: ['ROLE_ASSIGNED', 'ROLE_REVOKED', 'PERMISSION_GRANTED', 'PERMISSION_REVOKED']
      }
    }
  });
...
app.delete(
  ...
  requirePermission('project:delete'), // Must have permission
  async (req, res) => {
...
async function checkInheritedPermission(
  ...
  if (resource.parentResourceType && resource.parentResourceId) {
    ...
    const parentPermission = `${resource.parentResourceType}:${permission.split(':')[1]}`;
...
async function checkPermissions(
  ...
  for (const permission of permissions) {
    results[permission] = checkDirectPermission(
      permission,
      memberPermissions
    );
  }
